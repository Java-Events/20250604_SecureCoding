...
public class SecureLoginSimulation
  ...
  public static void main(String[] args) {
    ...
    staticLogger().info("starting the registration");
    simulation.simulateRegistration(server, client);
    staticLogger().info("starting the login");
  ...
  public void simulateLoginProcess(Server server, Client client) {
    char[] userName = "sven.ruppert".toCharArray();
    char[] passwordForLogin = "secret".toCharArray();
  ...
  public static class Server implements HasLogger {
    ...
    private static final Map<String, Login> loginDatabase = new HashMap<>();
    private static final Map<String, char[]> registrationSalts = new HashMap<>();
    private static final Map<String, char[]> dynamicSalts = new HashMap<>();
    ...
    public char[] requestDynamicSalt(char[] userName) {
      ...
      if (loginDatabase.containsKey(String.valueOf(userName))) {
        ...
        dynamicSalts.put(String.valueOf(userName), dynamicSaltForLogin);
    ...
    public char[] requestStaticSalt(char[] userName) {
      ...
      if (loginDatabase.containsKey(String.valueOf(userName))) {
        ...
        char[] staticSalt = loginDatabase.get(String.valueOf(userName)).staticSalt();
        ...
        registrationSalts.put(String.valueOf(userName), generatedSalt);
    ...
    public void registerUser(char[] userName, char[] hashed) {
      if (loginDatabase.containsKey(String.valueOf(userName))) {
      ...
      } else if (registrationSalts.containsKey(String.valueOf(userName))) {
        ...
        Login login = new Login(
            userName,
            hashed,
            registrationSalts.get(String.valueOf(userName)));
        logger().info("userName2LoginMap.put {}", login);
        loginDatabase.put(String.valueOf(userName), login);
        logger().info("remove user {} from registrationSalts", userName);
        registrationSalts.remove(String.valueOf(userName));
      } else {
    ...
    public boolean authenticate(char[] userName, char[] hashedPassword) {
      ...
      Login login = loginDatabase.get(String.valueOf(userName));
      if (login == null) {
        logger().info("User {} not found", userName);
        return false;
      }

      char[] dynamicSaltForLogin = dynamicSalts.get(String.valueOf(userName));
    ...
    public record Login(char[] username, char[] passwordHash, char[] staticSalt) {
      ...
      public String toString() {
        return "Login{" +
               ...
               ", staticSalt=" + Arrays.toString(staticSalt) +
               '}';
      }
    }
